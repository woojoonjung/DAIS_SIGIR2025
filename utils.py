...
def get_table_embedding(entry, model, tokenizer):
...
def prepare_Xy(path, model, tokenizer, target='class', seed=42):
    ...
    # Extract embeddings
    X_train = [get_table_embedding(entry, model, tokenizer) for entry in train_data]
    X_test = [get_table_embedding(entry, model, tokenizer) for entry in test_data]
...
def train_eval_rf(X_train, X_test, y_train, y_test, seed=42):
    ...
    if len(set(y_test)) == 2:
        metrics = {
            ...
            "f1_score": f1_score(y_test, y_pred, pos_label="yes", average="binary", zero_division=0),
        }
    else:
        metrics = {
