...
def _hash_row(row):
    ...
    filtered_row = {k: v for k, v in row.items() if k not in ["genre", "actor", "category", "brand"]}
            ...
            if filename.endswith(".json"):
                ...
                with open(filepath, "r", encoding="utf-8") as f:
                    ...
                    for row in sampled_rows:
                        for key in ["genre", "actor", "category", "brand"]:
                            row.pop(key, None)
        ...
        for filename in json_files:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                if len(unseen_rows) > sample_num:
                    sampled_rows = sorted(random.sample(unseen_rows, sample_num), key=lambda x: json.dumps(x, sort_keys=True))
                else:
...
class JSONDataset(Dataset):
    ...
    def _serialize(self, json_obj):
        tokenized = self.tokenizer.tokenize(str(json_obj))
        ...
        def recurse_json(obj, parent_key=""):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    current_position += len(tokenized_key) + 1  # `:` token
